...
function postCreate():Void {
	...
	for (i => strum in previewStrumLine.members) {
		var skinName:String = SkinHandler.skinNameHelper(noteSkinList[noteSkinDropdown.index], false, true);
	...
	noteSkinDropdown.onChange = (index:Int) -> {
		for (i => strum in previewStrumLine.members) {
			var skinName:String = SkinHandler.skinNameHelper(noteSkinList[index], false, true);
			var skinData = SkinHandler.getSkinData(skinName);
			var prevAnim:String = strum.getAnim();
			changeSkin(strum, previewStrumLine, i, skinName, skinData.pixelEnforcement);
			strum.playAnim(prevAnim);

			var skinName:String = SkinHandler.skinNameHelper(splashSkinList[splashSkinDropdown.index], true, true);
